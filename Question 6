...
La méthode dht.readTemperature() récupère la température en °C.
La méthode dht.readHumidity() récupère le taux d'humidité en %.
Affichage des données sur le LCD :

La température est affichée sur la première ligne.
Le taux d'humidité est affiché sur la deuxième ligne.
Temporisation :

Les données de température et d'humidité sont affichées pendant 5 secondes, avec une mise à jour toutes les secondes.
